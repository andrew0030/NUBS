...
public class BlockUBSSapling extends BlockBush implements IGrowable, IMetaName, IHasModel
{	
    ...
    public BlockUBSSapling(String name) 
    {
		...
		setCreativeTab(Main.instance.blocktab);
		this.setSoundType(SoundType.PLANT);
		this.setDefaultState(this.blockState.getBaseState().withProperty(VARIANT, UBSEnumHandler.EnumType.CHAKRA_INFUSED).withProperty(STAGE, Integer.valueOf(0)));
	...
	{
		return this.getDefaultState().withProperty(VARIANT, UBSEnumHandler.EnumType.byMetadata(meta & 1)).withProperty(STAGE, Integer.valueOf((meta & 8) >> 3));
	}
	...
	public void generateTree(World world, Random rand, BlockPos pos, IBlockState state)
	{
		...
		switch((UBSEnumHandler.EnumType)state.getValue(VARIANT))
		{
		case CHAKRA_INFUSED:
			gen = new WorldGenChakraInfusedTree(false, false);
			break;
		case AVOCADO:
			gen = new WorldGenAvocadoTree(false, false);
			break;
		}
	...
	public void registerModels() 
	{
		...
		{
			Main.proxy.registerItemVariantRenderer(Item.getItemFromBlock(this), i, "sapling_" + UBSEnumHandler.EnumType.values()[i].getName(), "inventory");
		}
	}
}
